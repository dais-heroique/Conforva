        ...
        <CardContent>
          {!files || files.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-gray-500">{t.technicalFiles.empty}</p>
              <Link href="/dashboard/products/new" className="mt-3 inline-block">
                <Button size="sm">{t.technicalFiles.createProduct}</Button>
              </Link>
            </div>
          ) : (
                ...
                return (
                  <div key={file.id} className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-gray-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900">{product?.name ?? t.unknownProduct}</p>
                    ...
                    <div className="flex items-center gap-3">
                      <Badge variant={file.watermarked ? "warning" : "success"}>
                        {file.watermarked ? t.status.notValidated : t.status.validated}
                      </Badge>
                      ...
                      <Link href={`/dashboard/products/${file.product_id}/export`}>
                        <Button variant="outline" size="sm" className="gap-1">
                          <Download className="h-3.5 w-3.5" />{t.pdfBtn}
                        </Button>
                      </Link>
                    </div>
                  </div>
        ...
        <CardContent>
          {!riskAssessments || riskAssessments.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-gray-500">{t.riskAssessments.empty}</p>
            </div>
          ) : (
            <div className="space-y-3">
                ...
                return (
                  <div key={ra.id} className="flex items-center gap-4 p-4 rounded-xl border border-gray-100">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900">{product?.name ?? t.unknownProduct}</p>
                      <p className="text-xs text-gray-500">
                        V{ra.version} · {formatDate(ra.created_at)} · {ra.ai_model ?? "IA"}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Badge variant={
                      ...
                      <Badge variant={ra.validated_by_human ? "success" : "secondary"}>
                        {ra.validated_by_human ? t.status.validated : t.status.notValidated}
                      </Badge>
