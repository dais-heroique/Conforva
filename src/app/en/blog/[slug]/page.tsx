...
import { ARTICLES_EN, getArticleEn } from "@/lib/blog/articles-en"
import { getFrSlugForEn } from "@/lib/blog/translations-map"
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const url = `https://conforva.com/en/blog/${article.slug}`
  const frSlug = getFrSlugForEn(article.slug)
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: {
        "fr-FR": frSlug ? `https://conforva.com/blog/${frSlug}` : "https://conforva.com/blog",
        "en-US": url,
      },
    },
  }
...
export default async function BlogArticlePageEn({ params }: PageProps) {
  const { slug } = await params
  const article = getArticleEn(slug)
  if (!article) notFound()

  const articleIndex = ARTICLES_EN.findIndex(a => a.slug === slug)
  const nextArticle = ARTICLES_EN[articleIndex + 1]
  const prevArticle = ARTICLES_EN[articleIndex - 1]
  ...
  const relatedArticles = ARTICLES_EN
  ...
  const jsonLd = {
    ...
    "mainEntityOfPage": { "@type": "WebPage", "@id": `https://conforva.com/en/blog/${article.slug}` },
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://conforva.com/en" },
      { "@type": "ListItem", "position": 2, "name": "Blog", "item": "https://conforva.com/en/blog" },
      { "@type": "ListItem", "position": 3, "name": article.title, "item": `https://conforva.com/en/blog/${article.slug}` },
    ],
  }
    ...
    <div className="min-h-screen bg-[#08090C]">
      ...
      <main className="max-w-3xl mx-auto px-5 py-16">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
          ...
          <span>/</span>
          <Link href="/en/blog" className="hover:text-gray-300 transition-colors">Blog</Link>
          <span>/</span>
        ...
        <header className="mb-10">
          ...
          <div className="flex items-center gap-5 text-sm text-gray-500 pb-6 border-b border-white/10">
            <span>By <span className="font-medium text-gray-300">Conforva</span></span>
            <span>{formatDate(article.publishedAt)}</span>
            <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" />{article.readingTime} min read</span>
          </div>
        </header>
        ...
        <article>
          ...
          <div className="my-10 rounded-xl bg-white/5 border border-white/10 p-6">
            <h2 className="text-base font-bold text-white mb-2">In summary</h2>
        ...
        <div className="mb-10 rounded-2xl border border-white/10 bg-white/4 p-6">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-600 mb-4">Related free tools</p>
          <div className="grid sm:grid-cols-2 gap-3">
            <Link
              href="/en/price-comparison"
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-white group-hover:text-[#A78BFA] transition-colors">Price comparison tool</p>
                <p className="text-xs text-gray-500">Compare your price to a competitor's, for free</p>
              </div>
            </Link>
            <Link
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-white group-hover:text-[#A78BFA] transition-colors">Margin calculator</p>
                <p className="text-xs text-gray-500">Calculate your margin and ideal selling price</p>
              </div>
            </Link>
          </div>
        </div>
        ...
        {relatedArticles.length > 0 && (
          <div className="mb-10">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-600 mb-4">Related articles</h3>
            <div className="space-y-3">
        ...
        <div className="flex items-center justify-between pt-8 border-t border-white/10 gap-4">
          {prevArticle ? (
            <Link href={`/en/blog/${prevArticle.slug}`} className="group flex items-center gap-2 text-sm text-gray-400 hover:text-[#8B5CF6] transition-colors max-w-[48%]">
              <ArrowLeft className="h-4 w-4 shrink-0" />
              <span className="line-clamp-2 text-left">{prevArticle.title}</span>
            </Link>
          ) : <div />}
          {nextArticle ? (
            <Link href={`/en/blog/${nextArticle.slug}`} className="group flex items-center gap-2 text-sm text-gray-400 hover:text-[#8B5CF6] transition-colors max-w-[48%] text-right ml-auto">
        ...
        <div className="mt-8 text-center">
          <Link href="/en/blog" className="inline-flex items-center gap-2 text-sm text-[#8B5CF6] hover:underline">
